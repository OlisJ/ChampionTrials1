...
function loadImages() {
    ...
    blueGhostImage.src = "./shadow.gif";

    orangeGhostImage = new Image();
    orangeGhostImage.src = "./engineer.gif";

    pinkGhostImage = new Image();
    pinkGhostImage.src = "./hipster.gif";

    redGhostImage = new Image();
    redGhostImage.src = "./speedster.gif";
...
function draw() {
    ...
    // --- Ghosts bigger and centered ---
    const ghostScale = 1.5;
    for (let ghost of ghosts.values()) {
        const newWidth = ghost.width * ghostScale;
        const newHeight = ghost.height * ghostScale;
        const offsetX = (ghost.width - newWidth) / 2;
        const offsetY = (ghost.height - newHeight) / 2;
...
if (pacman.x < -tileSize) {
    pacman.x = boardWidth; // teleport from left → right
}
else if (pacman.x > boardWidth) {
    pacman.x = -tileSize; // teleport from right → left
}
